...
//npx hardhat run --network bsc_mainnet  scripts/mainnet/nft_test.js
//npx hardhat run  scripts/mainnet/nft_test.js
...
//nftEarnErc20Pool deployed to: 0x38f3d5c04F45Eb98B5B8171d1C41FdCE08F7eF4F
    ...
    const dsgNft = await ethers.getContractAt("DsgNft", "0x48105a0fCcDB4D0347C015E9c3c17AF5591f7A10");
    const invitePool = await ethers.getContractAt("InvitePool", "0x06C314C945bfbdF3Ee9220757278137A2afDC87a");
    const egc = await ethers.getContractAt("UsToken", "0xEf1Db65b04ABB6E6d9A1870BeF0B0197eF3121a2");
    const busd = await ethers.getContractAt("UsToken", "0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56");
    const met = await ethers.getContractAt("UsToken", "0x505CE1F97cD05518F97a4921ee3C3d66fE160d3b");
    const crystalNft = await ethers.getContractAt("DsgNft", "0x0d4E7B0158eEC38e99604ab4601fe4d1dB0f95F1");
    const stakePool = await ethers.getContractAt("NftEarnErc20Pool", "0x4E6D02bbC86aA068992bC457bC865800164c2105");
    const rewardPool = await ethers.getContractAt("NftRewardPool", "0x2d56cFb060c2aa4393CB44f9c651302575585d8e");
    const recharge = await ethers.getContractAt("Recharge", "0x49B620983A49f3fDeAF564A3855556141312E218");
    const whiteNft = await ethers.getContractAt("DsgNft", "0xF63D9A6Bd096eB2FFC271Ef62C35838A0E1413c0");
...
//     underlying_ = '0x0000000000000000000000000000000000001000'
//     comptroller_ = '0x3b056De20d662B09f73bDb28Ea6fa7b7aC82259C'
//     chef_ = '0x40cE0de5fA5AFD6d519f3aa273288e7c6ce54675'
//     interestRateModel_ = '0xFD9Fb604f3C120ec0d6467a70FD452C8801E4741'
//     exchange = '100000000000000000000000'
//     name_ = 'pFRA'
//     symbol_ = 'pFRA'
//     pid_borrow_ = 0
//     pid_supply_ = 1
//     decimals_ = 18
//     admin_ = '0x089151bC7617316Cc830f67bb4716aFde43Bf9e4'
//     implementation_ = '0x7f0bBdC966B063bC67Aa5623545FcaD4cDF3c985'
//     data = "0x"
//
//     data = web3.eth.abi.encodeParameters(['address', 'address', 'address', 'address', 'uint256', 'string', 'string', 'uint256', 'uint256', 'uint8', 'address', 'address', 'bytes'], [underlying_,comptroller_,chef_,interestRateModel_,exchange,name_,symbol_,pid_borrow_,pid_supply_,decimals_,admin_,implementation_,data]);
//     console.log(data)


  //  comp.address, accounts[0],perBlock, 0)
  //   data = web3.eth.abi.encodeParameters(['address'], ['0x32D097E3CC87Da6fbB48c8041eBFcb1A5E49dD6D']);
  //    console.log(data)
  //   return
    //0x6524bB61a27bc34CD230B4e0bfd3cCe71EC356df

// _lockNft = "0x910BcaB3C559a13514CdAdD6a8b2BF21B8dDB6a4"
//      tx = await dsgNft.setLockNft(_lockNft);
//      console.log(tx.hash.toString());
    // tx=   await whiteNft.setApprovalForAll(dsgNft.address,true);
    //   tx.wait()
    //   console.log(tx.hash.toString())
    //  bal = await  whiteNft.balanceOf(accounts[0])
    // console.log(bal.toString())
    // id = await whiteNft.tokenOfOwnerByIndex(accounts[0],0)
    // console.log(id.toString())
    // tx = await dsgNft.freeMint("0x1A7D0141d7C635C736D062e46e759D85C14f3a13",[id]);
    //   console.log(tx.hash.toString())
    // tx = await whiteNft.transferFrom(accounts[0],"0x1A7D0141d7C635C736D062e46e759D85C14f3a13",id.toString())
    // console.log(tx.hash.toString())
    // bal = await whiteNft.balanceOf("0x9888b9d33651497725fffd4f40e30d35222b5858");
    // console.log(bal.toString())
    ...
    //           "type": "uint256"
    //       }
    //   ], '');
    //   console.log(b);
...
//     console.log("team address:",owner.toString());
    ...
    // tx =  await  dsgNft.transferOwnership("0x6524bB61a27bc34CD230B4e0bfd3cCe71EC356df");
    //  console.log(tx.hash.toString())
//     tx = await recharge.transferOwnership(_newOwner);
//     console.log(tx.hash.toString())
//     _newOwner = "0xEBB9151Ef2af19dc6484aaa633C39A6e8AAEdF41"
...
//     console.log(tx.hash.toString())
    ...
    // a= await  stakePool.getFullUserInfo("0xed4efdec97092a2e7fb7cfcbb3a5d5324a0a0a8a",425);
    //  console.log(a.toString())

    // a= await  stakePool.getNfts("0xed4efdec97092a2e7fb7cfcbb3a5d5324a0a0a8a")
    // console.log(a.toString())
...
//    console.log(r.toString())
    ...
    // tx = await dsgNft.setWhiteNft("0xf63d9a6bd096eb2ffc271ef62c35838a0e1413c0");
    // console.log(tx.hash.toString());
    // tx = await dsgNft.setPrice(web3.utils.toWei("1").toString(), web3.utils.toWei("20").toString());
    // console.log(tx.hash.toString());
